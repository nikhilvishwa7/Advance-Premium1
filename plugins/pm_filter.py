...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, AUTH_GROUPS, PICS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, TUTORIAL, REQST_CHANNEL, IS_TUTORIAL, LANGUAGES, SEASONS, SUPPORT_CHAT, PREMIUM_USER,  STREAM_BIN, STREAM_URL, STREAM_SITE, STREAM_API, STREAM_HOW_DOWNLOAD
...
import os
req_channel = int(os.environ.get('CRAZY_REQUEST','-1001728907275'))
    ...
    try:
        ...
        if f_sub:
            try:
                ...
                manual = await manual_filters(bot, message)
                ...
                mks = await message.reply(
                    f"<b> ⚠️ Dear {message.from_user.mention} ❗ \n\n 🙁 Please join our channel to get access. Click the button below to join:</b>",
                    reply_markup=types.InlineKeyboardMarkup([
                        [types.InlineKeyboardButton("⛔ Join Channel ⛔", url=f_link)],
                        [types.InlineKeyboardButton("♻️ Try Again ♻️", callback_data='checkuser')]
                    ])
                )
...
async def pm_text(bot, message):
    ...
    kk = await message.reply_text(
         ...
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"telegram.me/Movie_Web_Series_Group_Official")],[InlineKeyboardButton("🧑‍💻  ʙᴏᴛ ᴏᴡɴᴇʀ ", url=f"https://telegram.me/OTTProviderBackup")]])
    )
    await asyncio.sleep(60)
    await kk.delete()
...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn.insert(0, [
            InlineKeyboardButton("⛔ ʜᴏᴡ ᴛᴏ ᴏᴘᴇɴ ᴛʜɪꜱ ʟɪɴᴋ ⛔", url=f"https://telegram.me/{temp.U_NAME}")
        ])
    else:
        btn = []
        btn.insert(0, 
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            #button code - url=stream_link if is_premium_user else hp_link)
            ...
            await log_msg.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ꜰᴀsᴛ ᴅᴏᴡɴʟᴏᴀᴅ 📥", url=stream_link),
                                                    InlineKeyboardButton('🖥️ ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ', url=page_link)]]))
        ...
        try:
            if query.from_user.id in ADMINS:
                ...
                reply_markup = InlineKeyboardMarkup(btn)
                ms =  await query.edit_message_text(f"<b> #ɢʀᴏᴜᴘ 🔒\n\n<u>🏷️ ɢʀᴏᴜᴘ / ᴄʜᴀᴛ ɪɴꜰᴏʀᴍᴀᴛɪᴏɴ </u>\n\n☎️ ᴄʜᴀᴛ ɪᴅ - {chatID} </b>", reply_markup=reply_markup)
            else:
                await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        except Exception as e:
            ms.edit(f"error - {e} ")
        ...
        if not free_trial_status:            
            ...
            await message.reply_sticker("CAACAgIAAxkBAAIBTGVjQbHuhOiboQsDm35brLGyLQ28AAJ-GgACglXYSXgCrotQHjibHgQ")
            ...
            new_text = "**🤣 you already used free now no more free trial. please buy subscription.**"
